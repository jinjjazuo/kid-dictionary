...
describe('config', () => {
  ...
  it('gives up on the dictionary lookup long before an AI call', () => {
    // kaikki answers a real word in about a second and 404s an unknown one
    // in under one, so the two calls cannot share one budget: a child's typo
    // would wait out the AI timeout before seeing "we don't know that word".
    // The bound mattered more under the old dictionaryapi.dev, which hung to
    // a Cloudflare 522 at ~20s rather than 404ing; it is kept as a guard.
